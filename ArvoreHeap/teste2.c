...
int main(int argc, char* argv[]){
    ...
    Heap* heapmax = construir_heap_max(apostador, quantidade_apostas);
    Heap* heapmin = construir_heap_min(apostador, quantidade_apostas);
    ...
    uint32_t maior_numero_acertos = 0;
    uint32_t menor_numero_acertos = 0;

    if (heapmax->tamanho > 0) {
        maior_numero_acertos = heapmax->apostadores[0].quantidade_acertos;
    }
    if (heapmin->tamanho > 0) {
        menor_numero_acertos = heapmin->apostadores[0].quantidade_acertos;
    }

    bool ainda_existem_maximos = (heapmax->tamanho > 0);
    bool ainda_existem_minimos = (heapmin->tamanho > 0);
    
    while (ainda_existem_maximos || ainda_existem_minimos) {

        if (ainda_existem_maximos && heapmax->apostadores[0].quantidade_acertos == maior_numero_acertos) {
            apostador_ganhador_max[quantidade_ganho_maximo++] = extrair_Max(heapmax);
            if (heapmax->tamanho == 0) {
                ainda_existem_maximos = false;
            }
        } 
        else {
            ainda_existem_maximos = false;
        ...
        if (maior_numero_acertos != menor_numero_acertos && ainda_existem_minimos && heapmin->apostadores[0].quantidade_acertos == menor_numero_acertos) {
             apostador_ganhador_min[quantidade_ganho_minimo++] = extrair_Min(heapmin);
             if (heapmin->tamanho == 0) {
                ainda_existem_minimos = false;
             }
        } 
        else {
            ainda_existem_minimos = false;
    ...
    if (quantidade_ganho_maximo > 0) {
        fprintf(output, "[%u:%u:%u]\n", quantidade_ganho_maximo, maior_numero_acertos, metade_maior / quantidade_ganho_maximo);
        for(int i = 0; i < quantidade_ganho_maximo; i++){
            fprintf(output, "%s\n", apostador_ganhador_max[i].chave);
        }
    }

    if (quantidade_ganho_minimo > 0) {
        fprintf(output, "[%u:%u:%u]\n", quantidade_ganho_minimo, menor_numero_acertos, metade_menor / quantidade_ganho_minimo);
