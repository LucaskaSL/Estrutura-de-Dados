...
//gcc -Wall lucassantanaleal_202400028946_dicionario.c -o lucassantanaleal_202400028946_dicionario
//./lucassantanaleal_202400028946_dicionario input.txt output.txt
...
Arvore* Inserir_No(Arvore* a, Palavras palavra_inserida){
    ...
    if (fator_balanceamento > 1) {
        // Se o filho esquerdo pende para a esquerda ou está reto (>=0), é Rotação Simples
        if (balanceamento(a->Esquerda) >= 0) {
            return rotacao_Direita(a);
        }
        // Se o filho esquerdo pende para a direita (<0), é Rotação Dupla
        else {
    ...
    if (fator_balanceamento < -1) {
        // Se o filho direito pende para a direita ou está reto (<=0), é Rotação Simples
        if (balanceamento(a->Direita) <= 0) {
            return rotacao_Esquerda(a);
        }
        // Se o filho direito pende para a esquerda (>0), é Rotação Dupla
        else {
